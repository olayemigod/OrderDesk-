-- SellerTray P1 audit hardening: narrow SECURITY DEFINER execution context.

-- This RPC already fully qualifies its table references. Remove public/pg_temp
-- from the SECURITY DEFINER search path to eliminate object-shadowing risk.
alter function public.sellertray_list_businesses_for_current_user()
  set search_path = '';

-- Keep the existing payment-gate behavior, but use the current JWT role claim
-- rather than the older auth.role() helper for distinguishing trusted service
-- callers from normal authenticated merchant users.
...
begin
  ...
  -- Trusted server workflows use the service_role JWT. Every other caller must
  -- be an authenticated member of the order tenant.
