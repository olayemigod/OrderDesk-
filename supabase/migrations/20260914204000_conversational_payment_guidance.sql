-- SellerTray QA hardening: keep customer payment guidance conversational while
-- preserving order references for audit and disambiguation.
    ...
    when 'order_accepted' then
      case
        when payment_methods is not null then format(
          'Order %s is confirmed with %s. Invoice: %s. Total: %s %s. Payment options: %s. Tell us how you want to pay, for example “cash on delivery” or “send account details”. You can also ask for the invoice PDF.',
          ...
          'Order %s is confirmed with %s. Invoice: %s. Total: %s %s. You can ask for the invoice PDF. We will let you know when it is ready.',
          new.public_order_id,business_name,coalesce(invoice_ref,new.public_order_id),
          order_currency,trim(to_char(coalesce(order_total,0),'FM999999999990.00'))
        )
      end
