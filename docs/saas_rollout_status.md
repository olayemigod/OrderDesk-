...
### S11M — reproducible mobile dependency install — IN PROGRESS
- Production audit found no committed `apps/mobile/package-lock.json`; CI therefore resolved transitive dependencies with `npm install` on every run.
- Local resolver attempts could not complete from the current execution network and no historical lockfile exists in the repository.
- A bounded one-time same-repository CI bootstrap is being used to commit the lockfile generated by the already-working GitHub runner.
- After the lockfile lands, the self-writing step and temporary write permission must be removed and normal CI must switch to `npm ci`.
