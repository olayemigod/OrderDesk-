...
AI parser service-usage metadata is also covered here as a conservative working treatment: SellerTray records server-side model/outcome/provider-status and token counts linked to the source-message identifier so ProcessEdge can measure reliability and unit economics. The telemetry does not contain another copy of the customer message, prompt, catalogue context or model output and is not generated by a mobile diagnostics/advertising SDK.
